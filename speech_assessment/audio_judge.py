...
def compute_similarity(query_audio_file: str, ref_audio_file: str) -> ScoreResponse:
    ...
    try:
        ...
        return ScoreResponse(message="the server can't load the user's recording.", status=500)
    ...
    try:
        ...
        return ScoreResponse(message="the server can't load synthetic audio", status=500)
    # Compute query's MFCC features and check if the audio is too long
    query_mfcc = librosa.feature.mfcc(y=query_time_series, sr=query_sample_rate, n_mfcc=13)
    try:
